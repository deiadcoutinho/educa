...
const App: React.FC = () => {
  ...
  const addSubject = () => {
    if (!newSubj.name) return alert("Por favor, digite o nome da matéria.");
  ...
  const removeSubject = (name: string) => {
    if (confirm(`Tem certeza que deseja excluir "${name}"? Todos os exercícios desta matéria serão apagados.`)) {
  ...
  const addExercise = () => {
    if (!newEx.title || !newEx.url || !newEx.subject) return alert("Preencha o título, o link e selecione a matéria!");
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <header className="bg-white border-b p-4 sticky top-0 z-50 shadow-sm flex justify-between items-center px-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Portal do Estudante</p>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {isAdmin ? 'Sair do Painel' : 'Modo Professor 👨‍🏫'}
          </button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-6">
        {isAdmin ? (
          <div className="space-y-8 animate-in fade-in duration-300">
            <h2 className="text-3xl font-black text-slate-800">Gerenciar Exercícios 🛠️</h2>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Gerenciar Matérias */}
              <section className="bg-white p-6 rounded-3xl shadow-sm border">
                <h3 className="text-xl font-bold mb-4">1. Matérias</h3>
                <div className="space-y-4">
                  <input 
                    placeholder="Nome da Matéria (Ex: Física)" value={newSubj.name}
                  ...
                  <div className="flex gap-4">
                    <select 
                      value={newSubj.color} onChange={e => setNewSubj({...newSubj, color: e.target.value})}
                      className="flex-1 border-2 p-3 rounded-xl bg-white outline-none"
                    >
                    ...
                    <input 
                      placeholder="Ícone (Emoji)" value={newSubj.icon}
                      onChange={e => setNewSubj({...newSubj, icon: e.target.value})}
                      className="w-24 border-2 p-3 rounded-xl text-center outline-none"
                    />
                  </div>
                  <button onClick={addSubject} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold">Adicionar Matéria</button>
                </div>
                <div className="mt-6 flex flex-wrap gap-2">
                  {subjects.map(s => (
                    <div key={s.name} className="bg-slate-50 border p-2 rounded-lg flex items-center gap-2">
                      <span>{s.icon} {s.name}</span>
                      <button onClick={() => removeSubject(s.name)} className="text-rose-500 font-bold px-1">×</button>
                    </div>
                  ))}
                </div>
              </section>
              ...
              {/* Gerenciar Links */}
              <section className="bg-white p-6 rounded-3xl shadow-sm border">
                <h3 className="text-xl font-bold mb-4">2. Adicionar Jogo/Link</h3>
                <div className="space-y-3">
                  ...
                  <input placeholder="Link do Exercício (URL)" value={newEx.url} onChange={e => setNewEx({...newEx, url: e.target.value})} className="w-full border-2 p-3 rounded-xl outline-none" />
                  ...
                  <div className="flex gap-2">
                    <select value={newEx.difficulty} onChange={e => setNewEx({...newEx, difficulty: e.target.value as any})} className="flex-1 border-2 p-3 rounded-xl bg-white outline-none">
                      <option value="Fácil">Fácil</option>
                      <option value="Médio">Médio</option>
                      <option value="Difícil">Difícil</option>
                    </select>
                    <input type="number" placeholder="XP" value={newEx.points} onChange={e => setNewEx({...newEx, points: parseInt(e.target.value) || 0})} className="w-20 border-2 p-3 rounded-xl outline-none" />
                  </div>
                  <button onClick={addExercise} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold">Salvar Exercício</button>
                </div>
                <div className="mt-6 space-y-2 max-h-40 overflow-y-auto">
                    {exercises.map(ex => (
                      <div key={ex.id} className="text-xs p-2 border rounded flex justify-between items-center bg-slate-50">
                        <span>[{ex.subject}] {ex.title}</span>
                        <button onClick={() => removeExercise(ex.id)} className="text-rose-500 font-bold">Remover</button>
                      </div>
                    ))}
                </div>
              </section>
            ...
            {!selectedSubject ? (
              <div className="animate-in fade-in duration-500">
                <div className="bg-indigo-600 rounded-[2rem] p-8 text-white mb-10 shadow-xl relative overflow-hidden">
                  <h3 className="text-2xl font-black mb-1">Progresso Diário</h3>
                  <div className="flex justify-between items-end mb-2">
                    <p className="text-sm opacity-80">{user.completedTodayIds.length} de {user.dailyGoal} concluídos</p>
                ...
                <h2 className="text-2xl font-black text-slate-800 mb-6 flex items-center gap-2">
                  <span className="w-2 h-8 bg-indigo-600 rounded-full"></span> Matérias Disponíveis
                </h2>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-4 mb-8">
                  <button onClick={() => setSelectedSubject(null)} className="w-12 h-12 bg-white border-2 rounded-2xl flex items-center justify-center text-slate-600 hover:border-indigo-500 transition-all font-bold text-xl">←</button>
                ...
                <div className="bg-white p-6 rounded-3xl border-l-4 border-indigo-500 shadow-sm mb-10 flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Dica do Tutor IA</p>
